...
function renderAdvancedDashboard() {
  ...
  switch (state.activeAdvancedTab) {
    ...
    case 'visualBuilder': if (typeof renderVisualBuilderTab === 'function') renderVisualBuilderTab(); break;
...
async function copyDiagnostics() {
  ...
  // Step 36: Visual Builder + Scenario Presets line. Numbers /
  // metadata only — never base64, never imageDataUrl, never the
  // full target text. Visual Builder never auto-saves, never
  // auto-runs, never clicks.
  let vbDiag = null;
  if (typeof getVisualBuilderDiagnostics === 'function') {
    try { vbDiag = getVisualBuilderDiagnostics(); } catch (e) { vbDiag = null; }
  }
  const vbPresetsAvail = vbDiag ? !!vbDiag.presetsAvailable : true;
  const vbPresetsCount = vbDiag ? (vbDiag.presetsCount | 0) : 0;
  const vbLastPreset   = vbDiag ? (vbDiag.lastUsedPresetId || 'none') : 'none';
  const vbLastDraft    = vbDiag ? (vbDiag.lastDraftType || 'none') : 'none';
  const vbDraftAvail   = vbDiag ? !!vbDiag.visualBuilderDraftAvailable : false;
  const vbActionType   = vbDiag ? (vbDiag.selectedActionType || 'simple_click') : 'simple_click';
  const vbMissing      = vbDiag ? (vbDiag.missingRequirementsCount | 0) : 0;
  const vbOv = (vbDiag && vbDiag.overlaySettings) ? vbDiag.overlaySettings : {};
  const vbLine = `Visual Builder: presetsAvailable=${vbPresetsAvail}, presetsCount=${vbPresetsCount}, lastUsedPresetId=${vbLastPreset}, lastDraftType=${vbLastDraft}, visualBuilderDraftAvailable=${vbDraftAvail}, selectedActionType=${vbActionType}, missingRequirementsCount=${vbMissing}, showRegion=${!!vbOv.showRegion}, showTemplateMatch=${!!vbOv.showTemplateMatch}, showTemplateTarget=${!!vbOv.showTemplateTarget}, showOcrBlocks=${!!vbOv.showOcrBlocks}, showOcrTarget=${!!vbOv.showOcrTarget}, showActionTarget=${!!vbOv.showActionTarget}, autoSavesScenarios=false, autoRunsScenarios=false, realClick=false, realOcr=false`;
  // Step 38 — OCR provider registry diagnostics. Architecture-only.
  // Numbers / counts / stable booleans only — never the full target
  // text, never an `imageDataUrl`, never a thumbnail, never PII.
  let opStatus = null;
  if (typeof getOcrProviderRegistryStatus === 'function') {
    try { opStatus = getOcrProviderRegistryStatus(); } catch (e) { opStatus = null; }
  }
  const opActiveId = opStatus ? (opStatus.activeProviderId || 'none') : 'none';
  const opActiveName = opStatus ? (opStatus.activeProviderName || 'none') : 'none';
  const opMockAvail = opStatus ? !!opStatus.mockProviderAvailable : true;
  const opTessAvail = opStatus ? !!opStatus.tesseractProviderAvailable : false;
  const opRealEnabled = opStatus ? !!opStatus.realOcrEnabled : false;
  const opRealAllowed = opStatus ? !!opStatus.realOcrAllowed : false;
  const opLangs = (opStatus && Array.isArray(opStatus.supportedLanguages)) ? opStatus.supportedLanguages.join('|') : 'ru|en|ru+en';
  const opSelfTest = opStatus && opStatus.lastProviderSelfTest ? opStatus.lastProviderSelfTest : null;
  const opSelfTestOk = opSelfTest ? !!opSelfTest.ok : 'never';
  const opSelfTestAt = opSelfTest ? opSelfTest.ranAt : 'never';
  const opSelfTestBlocks = opSelfTest && opSelfTest.details ? (opSelfTest.details.blocksCount | 0) : 0;
  const opSelfTestDur = opSelfTest && typeof opSelfTest.durationMs === 'number' ? opSelfTest.durationMs : 0;
  const opRegistryReady = opStatus ? !!opStatus.providerRegistryReady : true;
  const opLine = `OCR provider: activeProviderId=${opActiveId}, activeProviderName=${opActiveName}, mockProviderAvailable=${opMockAvail}, tesseractProviderAvailable=${opTessAvail}, realOcrEnabled=${opRealEnabled}, realOcrAllowed=${opRealAllowed}, supportedLanguages=${opLangs}, providerRegistryReady=${opRegistryReady}, lastProviderSelfTestOk=${opSelfTestOk}, lastProviderSelfTestAt=${opSelfTestAt}, lastProviderSelfTestBlocksCount=${opSelfTestBlocks}, lastProviderSelfTestDurationMs=${opSelfTestDur}, ocrEngineImplemented=false, tesseractAvailable=false, realOcr=false, realClick=false`;
  // Step 39 — Real OCR / Tesseract provider diagnostics. Reads
  // the provider shell + the registry view + the OCR feature
  // status. Booleans / counts / stable ids only — never pixel
  // data, never the full target text. Real OCR never auto-runs.
  let tessDiag = null;
  if (typeof getTesseractProviderDiagnostics === 'function') {
    try { tessDiag = getTesseractProviderDiagnostics(); } catch (e) { tessDiag = null; }
  }
  let tessRegStatus = null;
  if (typeof getTesseractProviderStatus === 'function') {
    try { tessRegStatus = getTesseractProviderStatus(); } catch (e) { tessRegStatus = null; }
  }
  let ocrFeatureStatus = null;
  if (typeof getOcrFeatureStatus === 'function') {
    try { ocrFeatureStatus = getOcrFeatureStatus(); } catch (e) { ocrFeatureStatus = null; }
  }
  const tDepPresent  = tessDiag ? !!tessDiag.tesseractDependencyPresent : true;
  const tProvAvail   = tessDiag ? !!tessDiag.tesseractProviderAvailable : false;
  const tProvEnabled = tessDiag ? !!tessDiag.tesseractProviderEnabled  : false;
  const tRealOcrFlag = ocrFeatureStatus ? !!ocrFeatureStatus.realOcr   : false;
  const tActiveProv  = tessDiag && tessDiag.activeOcrProvider          ? tessDiag.activeOcrProvider : 'mock';
  const tLastReady   = tessDiag && tessDiag.lastTesseractReadinessCheck ? tessDiag.lastTesseractReadinessCheck : 'never';
  const tLastErr     = tessDiag && tessDiag.lastTesseractError         ? tessDiag.lastTesseractError : 'none';
  const tEngineLoad  = tessRegStatus ? !!tessRegStatus.engineLoadable  : false;
  const tDisabledRsn = tessRegStatus && tessRegStatus.disabledReason   ? tessRegStatus.disabledReason : 'n/a';
  const realOcrLine = `Real OCR: tesseractDependencyPresent=${tDepPresent}, tesseractProviderAvailable=${tProvAvail}, tesseractProviderEnabled=${tProvEnabled}, tesseractEngineLoadable=${tEngineLoad}, realOcrFeatureFlag=${tRealOcrFlag}, activeOcrProvider=${tActiveProv}, realOcrAutoRun=false, lastTesseractReadinessCheck=${tLastReady}, lastTesseractError=${tLastErr}, tesseractDisabledReason=${tDisabledRsn}, realOcr=false, realClick=false`;
  // Step 42 — Smart Beta health snapshot. Compact line listing
  // every smart-features readiness boolean plus the release-
  // blockers count. Booleans only — no pixel data, no PII.
  let smartBeta = null;
  if (typeof getSmartBetaHealth === 'function') {
    try { smartBeta = getSmartBetaHealth(); } catch (e) { smartBeta = null; }
  }
  const sbHealth = smartBeta || {};
  const smartBetaLine = `Smart beta: screenCaptureReady=${!!sbHealth.screenCaptureReady}, regionSelectorReady=${!!sbHealth.regionSelectorReady}, templatesReady=${!!sbHealth.templatesReady}, templateMatchingReady=${!!sbHealth.templateMatchingReady}, imageClickReady=${!!sbHealth.imageClickReady}, ocrMockReady=${!!sbHealth.ocrMockReady}, tesseractProviderReady=${!!sbHealth.tesseractProviderReady}, textClickReady=${!!sbHealth.textClickReady}, visualBuilderReady=${!!sbHealth.visualBuilderReady}, presetsReady=${!!sbHealth.presetsReady}, realClicksEnabled=false, releaseBlockersCount=${(sbHealth.releaseBlockersCount | 0)}, simulationOnly=true, realClick=false`;
